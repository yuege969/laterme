...
async function saveSetting(patch: Partial<AppSettings>): Promise<void> {
  try {
    await runtime.sendMessage({ type: 'SAVE_SETTINGS', payload: patch });
...
async function loadStats(): Promise<void> {
  try {
    ...
    const withNotes = metas.filter((m: { note: string; status: string }) => m.note && m.note.trim() && m.status === 'active').length;
...
importFile.addEventListener('change', async (e) => {
  ...
  try {
    ...
    if (!confirm('导入将覆盖现有的所有数据，确定继续吗？')) return;

    await runtime.sendMessage({ type: 'IMPORT_DATA', payload: data });
...
clearBtn.addEventListener('click', async () => {
  if (!confirm('确定要清除所有 LaterMe 数据吗？\n\n这将删除所有备注和提醒记录。\n浏览器的原生书签不会受影响。')) return;
  if (!confirm('再次确认：真的要清除所有数据吗？此操作不可撤销。')) return;

  try {
    await runtime.sendMessage({
      type: 'IMPORT_DATA',
      payload: { bookmarks_meta: [], resurfacing_logs: [], settings: DEFAULT_SETTINGS },
    });
    showToast('所有数据已清除');
    setTimeout(() => window.location.reload(), 1500);
  } catch {
